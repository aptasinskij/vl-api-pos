...
library CameraLib {
    ...
    struct StartQRScan {
        ...
        uint256 sessionId;
        bool lights;
        function(uint256, string memory, string memory, string memory) external scanned;
    ...
    function createStartQRScan(address self, StartQRScan memory command) internal {
        ...
        Database(self).setUint256Function(keccak256(abi.encodePacked(START_QR_SCAN_SUCCESS, command.id)), command.success);
        Database(self).setUint256stringX3Function(keccak256(abi.encodePacked(START_QR_SCAN_SCANNED, command.id)), command.scanned);
    ...
    function retrieveStartQRScan(address self, uint256 _id) internal view returns (StartQRScan memory) {
        ...
        return StartQRScan({
            ...
            scanned: Database(self).getUint256stringX3Function(keccak256(abi.encodePacked(START_QR_SCAN_SCANNED, _id))),
            success: Database(self).getUint256Function(keccak256(abi.encodePacked(START_QR_SCAN_SUCCESS, _id))),
