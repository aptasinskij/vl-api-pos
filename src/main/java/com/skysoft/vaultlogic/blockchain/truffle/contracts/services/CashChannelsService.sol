...
import "../repositories/cash-in/CashInRepositoryApi.sol";
...
contract CashChannelsService is RegistryComponent {
    ...
    function openCashInChannel(uint256 sessionId) external returns(uint256) {
        ...
        uint256 channelId = CashInRepositoryApi(lookup(CASH_IN_REPOSITORY)).save(sessionId, application, uint256(CashInStatus.PENDING));
    ...
    function closeCashInChannel(uint256 sessionId, uint256 channelId) external {
        ...
        CashInRepositoryApi(lookup(CASH_IN_REPOSITORY)).setStatus(channelId, uint256(CashInStatus.HALF_CLOSED));
    ...
    function confirmOpen(uint256 channelId) external {
        CashInRepositoryApi cashInRepo = CashInRepositoryApi(lookup(CASH_IN_REPOSITORY));
